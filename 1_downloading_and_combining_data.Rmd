...
```{r}
#Grab only the name of the file by splitting out on forward slashes
splits1 <- str_split_fixed(links1,'/',5)

#Keep only the 8th column
dataset1 <- splits1[,5] 

#generate a file list for where the data goes
file_names1 <- paste0('data/',dataset1)

#Map version of the same forloop (downloading 2 files)
map2(links1,file_names1,download.file)
...
snow_reader_txt <- function(file){
  name = str_split_fixed(file,'/',2)[,2] %>%
    gsub('.txt','',.)
  df <- read_delim(file, delim = " ", col_names = headers) %>%
    mutate(file = name)
}
...
snow_full <- map_dfr(file_names1,snow_reader_txt)
